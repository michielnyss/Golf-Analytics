...
club_windows <- list(
  ...
  "Driver" = list(
    speed  = c(140, 180),   # km/h
    launch = c(10, 16),     # deg
    spin   = c(1800, 3000)  # rpm
  ),
...
plots <- lapply(names(club_windows), function(club) {
  launch_condition_triangle(ironData, club, club_windows)
})

clubs_in_data <- intersect(unique(ironData$`Club Type`), names(club_windows))

for (club in clubs_in_data) {
  print(launch_condition_triangle(ironData, club, club_windows))
}
